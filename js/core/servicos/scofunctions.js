...
*******************************************************************************/
function SetStatusCompleted(args)
{
	//var result = doLMSInitialize();
...
//				doLMSSetValue( "cmi.core.lesson_location", args );	  
			...
			doLMSSetValue( "cmi.core.lesson_status", args );
			result = doLMSCommit();
		}
}
	...
	else if (Tela.length<1) 
		{
			Tela = licao + ":" + telas;
				...
				if (ind<0) 
				{
					...
					//retorna a posicao do proximo divisor da string após o indice da licao referida, neste caso uma ","
...
function GetSuspendData() {
  ...
  if (doLMSGetLastError() != "0") {   
   alert("Ocorreu um erro e seus dados não poderam ser recuperados. Por favor, reinicie o curso."); 
   return "";
  }
  else {
   return texto;
...
function SetSuspendData(dados) {
 doLMSSetValue("cmi.suspend_data", String(dados));
  ...
  if (doLMSGetLastError() != "0") {   
   alert("Ocorreu um erro e seus dados não poderam ser gravados. Por favor, reinicie o curso."); 
 }
}
//
function GetStatus() {
